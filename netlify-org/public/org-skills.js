/* Powerus Org Chart — skills view data (updated June 2026)
 * Categorizes individuals by the skill they bring to the company,
 * cutting across the formal reporting tree. A person may appear in
 * exactly one skill bucket.
 *
 * Names are matched against ORG_TREE for role/dept lookup; if no match,
 * a fallback role/dept is used (for individuals not yet in the tree).
 */

window.ORG_SKILLS = [
  {
    group: "Office of the CEO",
    people: [
      { name: "Andrew Fox" },
      { name: "Brett Velicovich" },
      { name: "Olivia Mcquail" },
      { name: "Roman Vinfeld" },
      { name: "Amy Bove" },
      { name: "Andrew Schmidt" },
      { name: "Lily Monterroso" },
    ],
  },
  {
    group: "Revenue",
    people: [
      { name: "Bob Harward" },
      { name: "Charlie Keebaugh" },
      { name: "Aaron Smith" },
      { name: "Brian Baswell" },
      { name: "Ryder McKee" },
    ],
  },
  {
    group: "Talent and Real Estate",
    people: [
      { name: "Patrick O'Hara" },
      { name: "Jordan Fox" },
    ],
  },
  {
    group: "Government Relations",
    people: [
      { name: "Chris Pratt" },
      { name: "Hogan Gidley" },
      { name: "Jessica Lycos" },
      { name: "Laura Saunders" },
      { name: "Allen Rubin" },
    ],
  },
  {
    group: "Legal",
    people: [
      { name: "Jim Biehl" },
      { name: "Nicole Nan" },
      { name: "Matt Farr" },
    ],
  },
  {
    group: "Finance",
    people: [
      { name: "Ed Jordan" },
      { name: "Jake Norris" },
      { name: "Silas" },
      { name: "Addy Alderman" },
    ],
  },
  {
    group: "Operations",
    people: [
      { name: "EVP of Operations", fallback: { role: "EVP of Operations", type: "open", dept: "ops" } },
      { name: "Scott Wolff" },
      { name: "Max Keebaugh" },
      { name: "Daniel Lauer" },
    ],
  },
  {
    group: "Logistics",
    people: [
      { name: "Jared Paul" },
      { name: "Natalie Ross" },
    ],
  },
  {
    group: "Flight",
    people: [
      { name: "Nathan Reim" },
      { name: "Sam Cousins" },
      { name: "Corey Tapp" },
      { name: "Dax Neal" },
    ],
  },
  {
    group: "Marketing",
    people: [
      { name: "Michael \u201CSquid\u201D Buchanan" },
      { name: "Troy Curtis" },
      { name: "Ryan Donahue" },
      { name: "Caleb Wright" },
      { name: "Erik Long" },
      { name: "Defiance Analytics", fallback: { role: "ETF Marketing & Social Media", type: "contract", dept: "marketing" } },
    ],
  },
  {
    group: "Communications",
    people: [
      { name: "Michael Sinensky" },
    ],
  },
  {
    group: "Investor Relations",
    people: [
      { name: "Jason Assad" },
    ],
  },
  {
    group: "Hardware Engineering",
    people: [
      { name: "Andrew Valkenburg" },
      { name: "Maxim Eschenazy" },
      { name: "Thejas Aradhya" },
      { name: "Shivansh Agrawal" },
      { name: "Noah Lambert" },
      { name: "Jason Barahona" },
      { name: "Jonathan Barahona" },
      { name: "Boris Illev" },
      { name: "Maxim Poe" },
    ],
  },
  {
    group: "Software / Tech",
    people: [
      { name: "Mohammed Farooq" },
      { name: "Josef Dahari" },
      { name: "Lo Dominguez" },
    ],
  },
  {
    group: "Manufacturing",
    people: [
      { name: "Robert Sigmon" },
      { name: "Brent Anderson" },
    ],
  },
  {
    group: "R&D",
    people: [
      { name: "Ziv Marom" },
      { name: "Jeremy Schnipke" },
    ],
  },
  {
    group: "AGH",
    people: [
      { name: "AGH Caretaker", fallback: { role: "Caretaker", type: "open", dept: "agh" } },
    ],
  },
];

// ── Skill → slug + helpers ─────────────────────────────────────
window.ORG_SKILL_SLUG = function(label) {
  return (label || '')
    .toLowerCase()
    .replace(/&/g, ' ')
    .replace(/[/]/g, ' ')
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
};

// Skill palette — one color per skill group.
window.ORG_SKILL_COLORS = {
  "office-of-the-ceo":    "#FF6200",
  "revenue":              "#C8344C",
  "talent-and-real-estate": "#C9A427",
  "government-relations": "#5C4FA8",
  "legal":                "#7D3FA6",
  "finance":              "#4A5A6A",
  "operations":           "#2C8A4A",
  "logistics":            "#8E7A2A",
  "flight":               "#1E8FB8",
  "marketing":            "#C1428A",
  "communications":       "#B7501E",
  "investor-relations":   "#5E9D2C",
  "hardware-engineering": "#2B2925",
  "software-tech":        "#2864C3",
  "manufacturing":        "#7A5C4A",
  "r-d":                  "#1E9E92",
  "agh":                  "#6B7A8D",
};

// Name → skill slug map (built from ORG_SKILLS at load time).
window.ORG_SKILL_OF = (() => {
  const map = new Map();
  (window.ORG_SKILLS || []).forEach(g => {
    const sl = window.ORG_SKILL_SLUG(g.group);
    g.people.forEach(p => map.set(p.name, sl));
  });
  return (name) => name ? (map.get(name) || '') : '';
})();
